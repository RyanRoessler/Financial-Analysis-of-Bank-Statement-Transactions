...
def filter_financial(cc_statement: Union[str, IO], dc_statement: Union[str, IO], filter_for: str, exclude: Union[str, None] = None) -> str:
    ...
    # Get average weekly cost
    num_weeks = tot_days / 7
    weekly_cost = round(total_cost / num_weeks, 2) if num_weeks > 0 else 0
        
    # Get average monthly cost
    num_months = tot_days / 30.44  # Average month length
    monthly_cost = round(total_cost / num_months, 2) if num_months > 0 else 0
    
    
    # Construct the summary string output
    summary = (
        f'Total days from {first_day.date()} to {last_day.date()}: {tot_days}\n'
        f'Total weeks: {num_weeks}\n'
...
def main():
    # Statements
    cc_statement = r"C:\Users\ryanr\OneDrive\Desktop\Life\Financial\Bank Statements\Credit Card - 7515_07-01-2024_03-01-2025.csv"
    dc_statement = r'C:\Users\ryanr\OneDrive\Desktop\Life\Financial\Bank Statements\Checking - 8508_07-01-2024_03-01-2025.csv'
    # Show statements
    #print(show_statements(cc_statement, dc_statement))
    
    # Gas
    # print(filter_financial(cc_statement, dc_statement, "fuel|gas|love's|circle|7-eleven|loaf|conoco|kum&go|QT|pilot|petro|everyday store", None))
    # Coffee
    # print(filter_financial(cc_statement, dc_statement, "starbucks|human b|coffee|sweet sinsations", None))
    # Clothes
    # print(filter_financial(cc_statement, dc_statement, "old navy|mens wearhouse", None))
    print(filter_financial(cc_statement, dc_statement, "wal-mart", None))
